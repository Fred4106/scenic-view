...
public class ScenicView extends Application {
    ...
    public static ScenicViewGui show(final Scene target) {
        return show(target.getRoot());
    }

    public static ScenicViewGui show(final Parent target) {
        if (target == null) {
            return null;
        }
        ...
        ScenicViewGui gui = new ScenicViewGui(updateStrategy, stage);
        ScenicViewGui.show(gui, stage);
        return gui;
    }
